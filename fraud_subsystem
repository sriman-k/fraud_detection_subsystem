...
transactions_file = "/Users/KundaSwami_1/Downloads/Synthetic_Fraud_Dataset.csv"
txn_df = pd.read_csv(transactions_file, parse_dates=['timestamp'])

# Load MCC dataset
mcc_file = "/Users/KundaSwami_1/Downloads/mcc_codes.csv"
...
# Rule E: Transactions at unusual times (expanded to 0-6 AM)
txn_df['rule_time'] = txn_df['txn_hour'].isin([0, 1, 2, 3, 4])

# Rule F: ML-based anomaly detection
txn_df['rule_ml_anomaly'] = txn_df['iso_anomaly'] == -1

# Rule Gm: Rare merchants
